...
struct AkOutputSettings
{
	...
	AkChannelConfig	channelConfig;	///< Channel configuration for this output. Call AkChannelConfig::Clear() to let the engine use the default output configuration.  Hardware might not support the selected configuration.

	AkCreatePluginCallback pfSinkPluginFactory;	///< Custom sink plugin factory. Pass the creation function of your implementation of AK::IAkSinkPlugin. Pass NULL to use built-in sinks (default).
};
...
namespace AK
{
	...
	namespace SoundEngine
	{
		...
		/// applicationWillEnterForeground: method. See the IntegrationDemo's application delegate for an example. It is designed to handle several interruptions incurred by the suspend and
