    ...
    )
{
	...
	out_deviceData.uMaxCachePinnedBytes = m_uCurrentCachePinnedData;
	m_uNumBytesTransferredFromLowLevel = 0;
	m_uTotalBytesTransferred = 0;
	m_uNumLowLevelRequests = 0;
	m_uNumLowLevelRequestsCancelled = 0;
...
void CAkDeviceBase::CountStreamsInTaskList(TaskList& in_listTasks)
{
    ...
    while ( it != in_listTasks.End() )
    {
        ...
        else if ( (*it)->IsProfileReady() ) // Review: This may affect the visibility of caching streams.
        {
    ...
    )
{
	...
	// Heuristics.
	m_fThroughput	= AkMax( in_heuristics.fThroughput, AK_MINIMAL_THROUGHPUT );
...
{
	if ( in_heuristics.priority < AK_MIN_PRIORITY ||
         in_heuristics.priority > AK_MAX_PRIORITY )
    {
	...
	else	// looping changed
	{
		...
		if ( m_fThroughput != fNewThroughput
...
AkReal32 CAkAutoStmBase::EffectiveDeadline()
{
	...
	AKASSERT( m_fThroughput >= AK_MINIMAL_THROUGHPUT );
	return m_uVirtualBufferingSize / m_fThroughput;
}
