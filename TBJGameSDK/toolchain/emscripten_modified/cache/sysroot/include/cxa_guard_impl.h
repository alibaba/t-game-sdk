...
extern "C" {
#endif
int pthread_mutex_lock_default();
int pthread_mutex_unlock_default();
int pthread_cond_wait_default();
int pthread_cond_broadcast_default();
#ifdef __cplusplus
}
#endif
...
typedef uint32_t PTHREAD_CXXABI_ID;
...
//                       PlatformGetThreadID
//===----------------------------------------------------------------------===//
#if defined(__JENOVA_DEFINED_pthread_t) && defined(_LIBCPP_HAS_THREAD_API_PTHREAD)
PTHREAD_CXXABI_ID PlatformThreadID()
{
  return (std::__libcpp_thread_get_current_id())._id;
}
#elif defined(__APPLE__) && defined(_LIBCPP_HAS_THREAD_API_PTHREAD)
...
struct GuardObject {
  ...
  explicit GuardObject(uint64_t* g)
      : base_address(g), guard_byte_address(reinterpret_cast<uint8_t*>(g)),
        init_byte_address(reinterpret_cast<uint8_t*>(g) + 1),
        thread_id_address(reinterpret_cast<PTHREAD_CXXABI_ID*>(g) + 1) {}
...
public:
  ...
  PTHREAD_CXXABI_ID* const thread_id_address;
...
//  bool unlock() { return std::__libcpp_mutex_unlock(&mutex); }
    ...
    bool lock() { return pthread_mutex_lock_default(); }
    bool unlock() { return pthread_mutex_unlock_default(); }
...
//  bool broadcast() { return std::__libcpp_condvar_broadcast(&cond); }
  ...
  bool wait(LibcppMutex& mut) {
    return pthread_cond_wait_default();
  }
  bool broadcast() { return pthread_cond_broadcast_default(); }
...
template <class Mutex, class CondVar, Mutex& global_mutex, CondVar& global_cond,
          PTHREAD_CXXABI_ID (*GetThreadID)() = PlatformThreadID>
...
private:
  ...
  LazyValue<PTHREAD_CXXABI_ID, GetThreadID> current_thread_id;
...
template <void (*Wait)(int*, int) = PlatformFutexWait,
          ...
          PTHREAD_CXXABI_ID (*GetThreadIDArg)() = PlatformThreadID>
