...
class libc(AsanInstrumentedLibrary, MuslInternalLibrary, MTLibrary):
  ...
  def get_files(self):
    ...
    ignore += [
        ...
        'memcmp.c', 'memchr.c', 'memmem.c',
        'strcpy.c', 'strncpy.c', 'strcat.c','strncat.c',
        'strcmp.c', 'strncmp.c', 'strchr.c', 'strrchr.c',
        'strstr.c', 'strlen.c', 'strdup.c', 'strndup.c', 
        'sin.c', 'sinf.c', 'asin.c', 'asinf.c',
        'cos.c', 'cosf.c', 'acos.c', 'acosf.c',
        'tan.c', 'tanf.c', 'atan.c', 'atanf.c', 'atan2.c', 'atan2f.c',
...
class libcxxabi(NoExceptLibrary, MTLibrary):
  ...
  def get_cflags(self):
    ...
    if not self.is_mt:
      cflags.append('-D_LIBCXXABI_HAS_JENOVA_THREADS')
    if self.eh_mode == Exceptions.NONE:
