...
def is_wasm_builtin_project(name):
    return name in __project_wasm_builtin_projects
...
class TBJXCodeExporter(object):
    ...
    def _export_public_compile_preprocessor(self, f, target):
        ...
        if clang_cxx_library is not None and len(clang_cxx_library) > 0:
    ...
    def _add_cmake_dependency(self, target, dep):
    ...
    def _export_one_target(self, f, target):
        ...
        if deps is not None and len(deps) > 0:
            for dep_hashcode in deps:
                ...
                if self._the_first_exporter is not None:
                    dep._the_first_exporter = self._the_first_exporter
                else:
                    dep._the_first_exporter = self

                self._add_cmake_dependency(target, dep)
                if self._the_first_exporter is not None:
                    self._the_first_exporter._add_cmake_dependency(target, dep)
    ...
    def export_cmake_dependencies(self, f):
        ...
        actual_has_dep = False
        for target_name, deps in self.__cmake_dependency.items():
            for dep in deps:
                dep_name = dep.get_export_target_name()
                if is_wasm_builtin_project(dep_name):
                    continue
                if not actual_has_dep:
                    actual_has_dep = True
                    f.write('\nadd_dependencies(%s' % stripped_name(target_name))

                f.write(' %s' % stripped_name(dep_name))
