...
def generate_wat_if_avaiable(cmake_file_dir, build_file_dir):
    ...
    if os.path.exists(wasm2wat):
        ...
        os.system("%s %s -o %s --enable-memory64" % (wasm2wat, wasm_path, wat_path))
...
if __name__ == '__main__':
    ...
    script_root_base = os.path.basename(os.path.dirname(script_root))
    emscripten_root = None
    if script_root_base == "taoemsdk":
    ...
    cmake_command = '%s cmake -DCMAKE_LIBRARY_ARCHITECTURE="wasm64-emscripten" -DCMAKE_CXX_FLAGS="${CMAKE_CXX_FLAGS} %s" -DCMAKE_C_FLAGS="${CMAKE_C_FLAGS} %s" ' \
