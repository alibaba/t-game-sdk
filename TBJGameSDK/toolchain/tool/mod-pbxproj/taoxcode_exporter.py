...
class TBJXCodeExporter(object):
    ...
    def search_config(self, config):
        ...
        self._target_copy_include_dir = os.path.abspath(os.path.join(self._cmake_output_dir, "gen_cmake_include_internal", stripped_name(self._export_target_name)))
    ...
    def _export_buildphase_source(self, f, target, buildPhase):
        ...
        if target_type == TARGET_APPLICATION:
            ...
            f.write('set_target_properties(%s PROPERTIES RUNTIME_OUTPUT_DIRECTORY "${CMAKE_BINARY_DIR}/bin")\n' % stripped_name(cmake_target_name))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target', dest="exportTarget", action='store', help='export target. auto search default, if more than one target. need specify a target.')
    ...
    parser.add_argument('--keep_objc', dest='keep_objc', action='store_true', help=".m(m) does not pch, besides objc awalys use mac framework. if you want keep objc, make sure do not use pch, add custom framework path first (as emscripten will has it's own sysroot, so ${OS_SYSTEM_ROOT} is no long in xcode sdk dir)!")
